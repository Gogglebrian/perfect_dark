		...
		if (bodymodeldef->skel == &g_SkelChr) {
			...
			if (node != NULL) {
				if (headnum < 0) {
					...
					if (!bvIsBotVarietyActive() && g_HeadsAndBodies[bodynum].canvaryheight && varyheight) { // @botvariety: if bot variety is on, we'll handle this elsewhere on each spawn, so don't bother doing it here
