		...
		if (bodymodeldef->skel == &g_SkelChr) {
			...
			if (node != NULL) {
				if (headnum < 0) {
					...
					if (!botvarietyIsActive() && g_HeadsAndBodies[bodynum].canvaryheight && varyheight) { // if bot variety is on, we'll handle this elsewhere on each spawn, so don't bother doing it here
